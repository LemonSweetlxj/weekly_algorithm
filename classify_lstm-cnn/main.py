...
parser = argparse.ArgumentParser(description='LSTM-CNN for SHIKAN classify')
...
## get char embeddings
word2id = read_dictionary(os.path.join('.', args.train_data, 'word2idPer.pkl'))
...
test_path = os.path.join('.', args.train_data, args.test_file)
...
## train mode
if args.mode == "train":
    model = LSTMCNN(args, embeddings, tag2label, word2id, paths, config=config)
    model.build_graph()
